    ...
    for j_folder in j_subfolders:
        ...
        for key in all_keys:
            ...
            try:
                ...
                if base_key == "params":
                    results["params"] = data_list[0].get("params")
                    continue
...
def avarage_plots_seeds(folder, plot_variance=True):
    ...
    for j_folder in j_subfolders:
        ...
        if L == 4:
            ...
            fidelity_mean = loaded_data.get('fidelity_sampled_NQS_DMRG_mean')
            fidelity_var = loaded_data.get('fidelity_sampled_NQS_DMRG_var')
            amp_overlap_mean = loaded_data.get('Overlap_amp_NQS_DMRG_mean')
            amp_overlap_var = loaded_data.get('Overlap_amp_NQS_DMRG_var')
            sign_overlap_mean = loaded_data.get('Overlap_sign_NQS_DMRG_mean')
            sign_overlap_var = loaded_data.get('Overlap_sign_NQS_DMRG_var')

            #if fidelity_mean is not None:
            #    Plot_Sign_Err_Amplitude_Err_Fidelity(amp_overlap_mean, fidelity_mean, sign_overlap_mean, folder_to_plot, one_avg = "avg", plot_variance=plot_variance, error_var=amp_overlap_var, fidelity_var=fidelity_var, sign_err_var=sign_overlap_var)
