...
from DMRG.Fidelities import Fidelity_exact
...
if __name__ == "__main__":
    ...
    for n_iter in n_iter_values:
        ...
        # --- Calculate Exact Fidelity (RBM vs DMRG) ---
        fidelity = Fidelity_exact(RBM_vstate, DMRG_vstate)
        exact_fidelities.append(fidelity)
        print(f"Exact Fidelity (RBM vs DMRG): {fidelity:.6f}")

    # --- Plotting the results ---
    fig, ax1 = plt.subplots(figsize=(12, 7))

    # Plot Marshall signs on the primary y-axis (ax1)
    ax1.set_xlabel("Number of RBM Training Iterations (n_iter)")
    ax1.set_ylabel("Marshall Sign", color='tab:blue')
    ax1.errorbar(n_iter_values, [s.real for s in mcmc_signs], yerr=[s.imag for s in mcmc_signs], fmt='s--', color='tab:blue', label='RBM Sign (MCMC Sampling)', markersize=8, capsize=5)
    ax1.plot(n_iter_values, [s.real for s in dmrg_sampled_rbm_signs], 'o-', color='tab:cyan', label='RBM Sign (DMRG Importance Sampling)', markersize=8, linewidth=2)
    ax1.axhline(y=-sign_exact_gs, color='green', linestyle='-', linewidth=2, label=f'Exact GS Sign = {sign_exact_gs:.4f}')
    ax1.tick_params(axis='y', labelcolor='tab:blue')
    ax1.set_xscale('log')

    # Create a secondary y-axis for fidelity
    ax2 = ax1.twinx()
    ax2.set_ylabel("Fidelity", color='red')
    ax2.plot(n_iter_values, exact_fidelities, 'd-', color='red', alpha=0.3, label='Fidelity (Exact RBM vs DMRG)', markersize=8)
    ax2.tick_params(axis='y', labelcolor='red')
    ax2.set_ylim(bottom=0, top=1.0) # Fidelity is between 0 and 1

    # Final plot adjustments
    fig.suptitle(f"Sign & Fidelity vs. RBM Training (L={model_params['Lx']}, J2={model_params['J2']})", fontsize=16)
    ...
    if valid_signs:
        ax1.set_ylim(bottom=min(valid_signs) - 0.1, top=max(valid_signs) + 0.1)

    # --- Save the final plot ---
    plot_output_dir = "DMRG/plot/Sign_vs_Iterations"
    os.makedirs(plot_output_dir, exist_ok=True)
    final_plot_filename = os.path.join(plot_output_dir, f"sign_and_fidelity_vs_n_iter_L{model_params['Lx']}_J2_{model_params['J2']}.png")
