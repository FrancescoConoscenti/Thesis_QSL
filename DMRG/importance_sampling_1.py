...
if __name__ == "__main__":
    ...
    # --- Marshall Sign and Amplitude Overlap Calculations ---
    print("\n--- Calculating Marshall Sign and Amplitude Overlaps ---")

    # Get Hilbert space from RBM vstate
    hi = RBM_vstate.hilbert

    # Treat DMRG state as the 'exact' ground state for comparison
    dmrg_ket = dmrg_array.reshape(-1, 1)

    # --- Calculate Marshall Sign for RBM ---
    # Marshall_Sign_full_hilbert expects a directory of models.
    # We'll create a temporary directory and save the current RBM state there.
    with tempfile.TemporaryDirectory() as temp_dir:
        temp_model_dir = os.path.join(temp_dir, "models")
        os.makedirs(temp_model_dir)
        temp_model_path = os.path.join(temp_model_dir, "model_0.mpack")

        # Save the current RBM state to the temporary file
        with open(temp_model_path, "wb") as f:
            bytes_out = flax.serialization.to_bytes(RBM_vstate.variables)
            f.write(bytes_out)

        # Now call the function with the temporary directory path
        # It will load "model_0.mpack" and perform the calculation.
        sign_rbm_arr, signs_rbm_arr = Marshall_Sign_full_hilbert(RBM_vstate, temp_dir, hi)
        sign_rbm = sign_rbm_arr[0]
        signs_rbm = signs_rbm_arr[0]

    # --- Calculate Marshall Sign for DMRG and Overlaps ---
    sign_dmrg, signs_dmrg = Marshall_Sign_exact(dmrg_ket, hi)
    print(f"Marshall Sign (RBM, full Hilbert): {sign_rbm:.6f}")
    print(f"Marshall Sign (DMRG, exact): {sign_dmrg:.6f}")

    amp_overlap = Amp_overlap_configs(dmrg_ket, RBM_vstate, hi)
    sign_overlap = Sign_overlap(dmrg_ket, signs_rbm.reshape(1, -1), signs_dmrg)
    print(f"Amplitude Overlap <|ψ_RBM| | |ψ_DMRG|>: {amp_overlap:.6f}")
    print(f"Sign Overlap: {sign_overlap[0]:.6f}")
