...
def update_orbitals(shape, h, phi, n_elecs, positions, nearest_neighbors, lattice, print_E=False):
    h, phi = jnp.abs(h), jnp.abs(phi)
    N = len(positions)
    def Hk(sigmaz):
    ...
    def initialize_real(num_particles,sigmaz):
      ...
      mat, E, us, energies = x
      if print_E: jax.debug.print("MF energy: {E}", E=E)
      return mat
...
def update_orbitals_apbc(shape, h, phi, n_elecs, positions, nearest_neighbors, lattice, print_E=False):
    h, phi = jnp.abs(h), jnp.abs(phi)
    N = len(positions)
    L = jnp.int16(jnp.sqrt(N))
    def Hk(sigmaz):
      bc = -1
      def hopping_x_conditions(inp):
          H, x, y, ix = inp
          #ix = int(((x + 1) % Lx) * Ly + y)
          flux = (-1)**(jnp.floor(x) + jnp.floor(y)) * 1j * phi * jnp.pi
          # apparently, in jax the commented code below doesn't work. This bit replaces it:
          coeff = jax.lax.select((jnp.abs(i-ix) == L), 1, bc)
          H = H.at[i, ix].add(-coeff*jnp.exp(flux))
          H = H.at[ix, i].add(-coeff*jnp.exp(-flux))
          # if jnp.abs(i-ix) == L:
          #   H = H.at[i, ix].add(-jnp.exp(flux))
          #   H = H.at[ix, i].add(-jnp.exp(-flux))
          # else:
          #   H = H.at[i, ix].add(-bc*jnp.exp(flux))
          #   H = H.at[ix, i].add(-bc*jnp.exp(-flux))
          return H
      ...
      def hopping_y_conditions(inp):
          ...
          flux = (-1)**(jnp.floor(x) + jnp.floor(y) + 1) * 1j * phi * jnp.pi
          # apparently, in jax the commented code below doesn't work. This bit replaces it:
          coeff = jax.lax.select(jnp.abs(i-iy) == 1, 1, bc)
          H = H.at[i, iy].add(-coeff*jnp.exp(flux))
          H = H.at[iy, i].add(-coeff*jnp.exp(-flux))
          # if jnp.abs(i-iy) == 1:
          #   H = H.at[i, iy].add(-jnp.exp(flux))
          #   H = H.at[iy, i].add(-jnp.exp(-flux))
          # else:
          #   H = H.at[i, iy].add(-bc*jnp.exp(flux))
          #   H = H.at[iy, i].add(-bc*jnp.exp(-flux))
          return H
    ...
    mf = jnp.block([[upmatrix, jnp.zeros(upmatrix.shape)], [jnp.zeros(dnmatrix.shape),dnmatrix]]).T
    #jax.debug.print("mf: {x}",x=mf)
    # print("mf shape: ", jnp.shape(mf))
    return mf



class GutzwillerWaveFunction(nn.Module):
  # take care of MF BC in _update_orbitals function
  # right now APBC
  n_elecs: int
  Lx: int
  Ly: int
  ...
  def _update_orbitals(self, key, shape):
    # Return zeros as a placeholder — mf must be set externally via
    # vstate.parameters before any sampling or evaluation (see maps.py scan loop).
    # Calling update_orbitals_apbc here triggers a JAX sharding error because
    # this initializer is traced inside apply_chunked's sharded context during
    # sampler initialisation.  The zeros are immediately overwritten by the
    # first new_pars["mf"] = update_orbitals_apbc(...) assignment in maps.py.
  ...
  def __call__(self,x):
    # returns 1/2 log pdf = log wavefunction
    # x is an array of dimensions n_chains x 2L**2. It stores n_chains states [0 1 0 1 1 ...]
    # x is a jax Tracer and cannot be accessed at runtime
    # jax.debug.print("state = {x}", x=x)
    ind1, ind2 = jnp.nonzero(x,size=x.shape[0]*self.n_elecs)  # ind1 = index of the chain, ind2 = actual position
    orbitals = jnp.repeat(jnp.expand_dims(self.mf,0),x.shape[0],axis=0)[ind1,ind2]
    # jax.debug.print("orbitals shape: {x}", x=jnp.shape(orbitals))
    orbitals = orbitals.reshape(-1,self.n_elecs,orbitals.shape[1])
    # jax.debug.print("orbitals shape 2: {x}", x=jnp.shape(orbitals))
    do = self.double_occupancy(x)
    sign, x = jnp.linalg.slogdet(orbitals)
    if self.double_occupancy_bool: 
      return x + jnp.log(sign + 0j)
    else:
      return x - 1e14*do + jnp.log(sign + 0j)