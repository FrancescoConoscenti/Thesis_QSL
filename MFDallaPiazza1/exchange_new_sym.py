...
class ParticleExchangeRule(ExchangeRule):
    ...
    def random_state(self, sampler, machine, params, state, rng):
      apply_machine = apply_chunked(machine.apply, in_axes=(None, 0), chunk_size=4096*2)

      def loop_body(val):
          n, x, neg_inf_mask, params, rng, sampler = val
          n += 1
          probs, _ = apply_machine(params, x, mutable=True)
          probs = probs.real
          neg_inf_mask = jnp.isneginf(probs) | jnp.isnan(probs) | (jnp.abs(probs) >= 1.e+12) 
          num_trues = jnp.sum(neg_inf_mask)
          #jax.debug.print("Number of inf values: {num}", num=(num_trues, jnp.sum(jnp.isnan(probs)), probs[10], neg_inf_mask[10]))
      ...
      probs = apply_machine(params, x, mutable=False)
      probs = probs.real
      neg_inf_mask = jnp.isneginf(probs) | jnp.isnan(probs) | (jnp.abs(probs) >= 1.e+12)
      ...
      x_rep = x[idx] 
      neg_inf_mask = jnp.repeat(jnp.expand_dims(neg_inf_mask, 1), x.shape[1], 1)
      probs = apply_machine(params, x_rep.reshape(-1,x_rep.shape[-1]), mutable=False)
    ...
    """def random_state_(self, sampler, rng):
        #half the samples, s.t. states + states_flipped = samples
        batch_size = sampler.n_batches//2
        ...
        n_up = (sampler.hilbert.n_fermions+1)//2
        n_dn = sampler.hilbert.n_fermions//2
        up_ind = indices[:,:n_up]
        dn_ind = indices[:, n_up:(n_up+n_dn)]+sampler.hilbert.n_orbitals
        
        states = jnp.zeros((batch_size, sampler.hilbert.size))
        states_flipped = jnp.zeros(states.shape)
        ...
        states_flipped = states_flipped.at[:,:sampler.hilbert.size//2].set(states[:,sampler.hilbert.size//2:])
        states_flipped = states_flipped.at[:,sampler.hilbert.size//2:].set(states[:,:sampler.hilbert.size//2])
        return jnp.concatenate((states, states_flipped), axis=0)"""
    ...
    def random_state_(self, sampler, rng):
        ...
        keys = jax.random.split(rng, batch_size)
        indices = jax.vmap(lambda key: jax.random.permutation(key, jnp.arange(sampler.hilbert.n_orbitals*2)))(keys)
        indices = indices[:,:sampler.hilbert.n_fermions]
        states = jnp.zeros((batch_size, sampler.hilbert.size))
        rows = jnp.arange(states.shape[0])
        
        #insert up and down spins
        states = states.at[(rows, indices.T)].set(1)
        return states
...
class tJExchangeRule_(MetropolisRule):
    ...
    def random_state(self, sampler, machine, params, state, rng):
      ...
      def loop_body(val):
          ...
          neg_inf_mask = jnp.repeat(jnp.expand_dims(neg_inf_mask, 1), x.shape[1], 1)
          rng = rng + sampler.n_batches*2
          #jax.debug.print("num up = {x}", x=sampler.hilbert.n_fermions//2-(-1)**n*jnp.ceil(n/2))
          x_rep = self.random_state_(sampler, rng, sampler.hilbert.n_fermions//2-(-1)**n*jnp.ceil(n/2))
          x = jax.lax.select(neg_inf_mask, x_rep, x)
          return n, x, neg_inf_mask, params, rng, sampler
      ...
      def loop_cond(val):
          n, _, neg_inf_mask, _, _, _ = val
          neg_inf_mask = neg_inf_mask[:,0]
          return jnp.logical_and(jnp.any(neg_inf_mask), jnp.sum(neg_inf_mask) > neg_inf_mask.shape[0]-1)

      x = self.random_state_(sampler, rng, sampler.hilbert.n_fermions//2)
    ...
    def random_state_(self, sampler, rng, num_up):
        #random sample with single occupancy
        batch_size = sampler.n_batches
        keys = jax.random.split(rng, batch_size+1)
        indices = jax.vmap(lambda key: jax.random.permutation(key, jnp.arange(sampler.hilbert.n_orbitals)))(keys[:batch_size])
        indices = indices[:,:sampler.hilbert.n_fermions]
        # indices = indices.at[:, num_up:].add(sampler.hilbert.n_orbitals)
        # We would like to use the above line, but it doesn't work in jax. Instead, we do the following:
        cond_mask = jnp.repeat(jnp.expand_dims(jnp.arange(sampler.hilbert.n_fermions), axis=0), batch_size, axis=0)
        indices = jnp.where(cond_mask>=num_up, indices+sampler.hilbert.n_orbitals, indices)
        # indices = jnp.where(indices>num_up, indices+sampler.hilbert.n_orbitals, indices)
        states = jnp.zeros((batch_size, sampler.hilbert.size))
        rows = jnp.arange(states.shape[0])
    ...
    def transition(rule, sampler, machine, parameters, state, key, σ, return_flipped=False):
        ...
        def _update_samples(key, σ, hoppable_clusters):
            ...
            #σp_flipped = jax.lax.cond(flip == 0, flip_true, flip_false, σp)
        ...
        σps, log_prob_corrs = _update_samples(keys, σ, hoppable_clusters)
