...
from HFDS_Heisenberg.Init_orbitals import Orbitals_phi, Orbitals
...
class HiddenFermion_phi(nn.Module):
  L: int
  network: str
  n_hid: int
  layers: int
  ...
  def setup(self):
    ...
    self.n_modes = 2 * N_sites
    self.n_elecs = N_sites
    self.orbitals = Orbitals_phi(self.L, N_sites, self.n_elecs, self.n_hid, self.MFinit, self.stop_grad_mf, self.bounds, self.phi, self.dtype, self.U)
    # FFNN architecture
    if self.network=="FFNN":
        self.hidden = [nn.Dense(features=self.features,use_bias=False,param_dtype=self.dtype) for i in range(self.layers)]
        self.output = nn.Dense(features=self.n_hid*(self.n_elecs + self.n_hid),use_bias=True,param_dtype=self.dtype)
