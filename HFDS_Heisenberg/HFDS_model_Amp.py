...
class HiddenFermion_amp(nn.Module):
  ...
  def setup(self):
    ...
    self.exact_log_amps = jnp.log(jnp.array(self.ket_gs, dtype=self.dtype) + 0j)
  ...
  def __call__(self,x):
    ...
    exact_log_amp = self.exact_log_amps[indices] #maybe you have to create multiple values, because log_sign is a vector, one value per symmetric samples

    log_psi_sym = exact_log_amp  + log_sign
