...
parser.add_argument("--N_iter_conv", type=int, default=5000, help="Number of iterations for the first phi (no symmetries)")
parser.add_argument("--N_iter_symm", type=int, default=5000, help="Number of iterations with symmetries (first phi only)")
parser.add_argument("--N_iter_adiabatic", type=int, default=500, help="Number of iterations for subsequent phis")
...
for idx, phi in enumerate(args.phi_list):
    ...
    folder_iter = N_total_phi
    if load_path is not None and idx == 0:
        folder_iter = previous_iter + N_total_phi
                ...
                if os.path.exists(load_save_model):
                    ...
                    if files:
                        ...
                        if load_path:
                            ...
                            old_log_path_load = os.path.join(load_seed_path, f"log_{phase_name}.pkl")
                            if not os.path.exists(old_log_path_load):
                            ...
                            if os.path.exists(old_log_path_load):
                                ...
                                try:
                                    ...
                                    old_log_data = (
                                        loaded_log if not old_log_data
                                        else helper.merge_log_data(loaded_log, old_log_data)
                                    )
