...
class tJExchangeRule_(MetropolisRule):
    ...
    def random_state_(self, sampler, rng):
        ...
        #insert up spins
        states = states.at[(rows, up_ind.T)].set(jnp.array(1, dtype=sampler.dtype))
        #insert down spins
        states = states.at[(rows, dn_ind.T)].set(jnp.array(1, dtype=sampler.dtype))
        return states
        
    def transition(rule, sampler, machine, parameters, state, key, σ, return_flipped=False):
        def flip_true(sigma):
            new_sigma = jnp.zeros(sigma.shape, dtype=sigma.dtype)
        ...
        def flip_single_true(sigma):
            ...
            new_sigma = new_sigma.at[ind].set(jnp.array(0, dtype=sigma.dtype))
            new_sigma = new_sigma.at[ind+sigma.shape[0]//2].set(jnp.array(1, dtype=sigma.dtype))
            return new_sigma
        n_chains = σ.shape[0]
        n_modes = σ.shape[1]
