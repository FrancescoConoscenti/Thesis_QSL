    ...
    for L in L_values:
        ...
        for var in variances:
            ...
            for name, model, hi, sampler in models_list:
                ...
                if var not in results[name]: results[name][var] = {'L': [], 'mean': [], 'err': [], 'params': []}
                
                results[name][var]['L'].append(L)
                results[name][var]['mean'].append(mean)
                results[name][var]['err'].append(err)
                results[name][var]['params'].append(param_count)
...
def plot_entropy_vs_L_hidden_size(n_seeds=10, n_samples=4096, models_to_plot=None, var=1):
    ...
    results = {} # results[model_name][size_label] = {'L': [], 'mean': [], 'err': [], 'params': []}
    ...
    for L in L_values:
        ...
        for size_label in ['Small', 'Medium', 'Large']:
            ...
            for name, model, hi, sampler in models_list:
                ...
                if name not in results: results[name] = {}
                if size_label not in results[name]: results[name][size_label] = {'L': [], 'mean': [], 'err': [], 'params': []}
                
                results[name][size_label]['L'].append(L)
                results[name][size_label]['mean'].append(mean)
                results[name][size_label]['err'].append(err)
                results[name][size_label]['params'].append(param_count)
            ...
            if models_to_plot is None or "ViT" in models_to_plot:
                ...
                if size_label not in results[name_x]: results[name_x][size_label] = {'L': [], 'mean': [], 'err': [], 'params': []}
                
                results[name_x][size_label]['L'].append(L)
                results[name_x][size_label]['mean'].append(mean)
                results[name_x][size_label]['err'].append(err)
                results[name_x][size_label]['params'].append(param_count_x)
    ...
    plot_groups = {
        ...
        "ViT": [k for k in results.keys() if "ViT" in k],
        "HFDS": [k for k in results.keys() if "HFDS" in k]
    }

    for group_name, model_names in plot_groups.items():
        if not model_names:
            continue
        ...
        plt.xlabel('Number of Spins N (L^2)')
        plt.ylabel('Renyi-2 Entropy')
        plt.title(f'Entanglement Entropy vs L ({group_name}, Varying Hidden Size, Unnormalized, Avg {n_seeds} seeds, samples={n_samples})')
        plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        
        filename = f"Entropy_vs_L_HiddenSize_Unnormalized_{group_name}.png"
        save_path = get_unique_path(save_dir, filename)
        plt.savefig(save_path)
        print(f"Plot ({group_name}) saved to {save_path}")
        plt.close()

def plot_entropy_vs_variance_hidden_size_map(n_seeds=10, n_samples=4096, models_to_plot=None, L_values=None):
    ...
    # Ranges
    variances = [0.01, 0.1, 1, 10, 50]
    if L_values is None:
        L_values = [4, 6, 8]
    elif isinstance(L_values, int):
        L_values = [L_values]
    ...
    for L in L_values:
        ...
        models_defs = {
            ...
            "HFDSrandom": {
                "param_name": "n_hid",
                "h_values":[2,  4,  8, 16],
                "builder": lambda h, init: HiddenFermion_ent(L=L, network="FFNN", n_hid=h, layers=1, features=64, MFinit="random", hilbert=hi_constrained, kernel_init=init, dtype=jax.numpy.complex128),
                "hilbert": hi_constrained,
                "sampler": nk.sampler.MetropolisExchange(hi_constrained, graph=g)
            },
            "HFDSFermi": {
                "param_name": "n_hid",
                "h_values": [2, 4, 8, 16],
                "builder": lambda h, init: HiddenFermion_ent(L=L, network="FFNN", n_hid=h, layers=1, features=16, MFinit="Fermi", hilbert=hi_constrained, kernel_init=init, dtype=jax.numpy.complex128),
                "hilbert": hi_constrained,
                "sampler": nk.sampler.MetropolisExchange(hi_constrained, graph=g)
            }
        }
        ...
        for model_name, config in models_defs.items():
            if models_to_plot is not None and model_name not in models_to_plot:
                continue
            print(f"Running sweep for {model_name} (L={L})...")
            ...
            for i, h in enumerate(h_vals):
                for j, var in enumerate(variances):
                    ...
                    print(f"  {model_name} L={L} h={h} var={var:.2f} -> S2={entropy_grid[i, j]:.4f}")
                    clean_up()
            
            # Un-normalize
            max_ent = (N // 2) * np.log(2)
            entropy_grid *= max_ent
            
            # Use index-based extent for non-linear axes
            extent = [0, len(variances), 0, len(h_vals)]
        ...
        if results:
            ...
            for ax, res in zip(axes, results):
                ...
                # Set ticks for non-linear axes
                ax.set_xticks(np.arange(len(res['variances'])) + 0.5)
                ax.set_xticklabels(res['variances'], rotation=45)
                ax.set_yticks(np.arange(len(res['h_vals'])) + 0.5)
                ax.set_yticklabels(res['h_vals'])
                
            fig.colorbar(im, ax=axes, label='Renyi-2 Entropy', location='right')
            fig.suptitle(f'Entanglement Entropy vs Variance & Hidden Size (N={N}, samples={n_samples}, seeds={n_seeds})')
            
            filename = f'Entanglement_Sweep_Variance_All_L{L}.png'
            save_path = get_unique_path(save_dir, filename)
            plt.savefig(save_path)
            print(f"Combined plot saved to {save_path}")
            plt.close(fig)

            # Group plots (ViT only, HFDS only)
            groups = {
                "ViT": ["ViTrandom", "ViTXavier"],
                "HFDS": ["HFDSrandom", "HFDSFermi"]
            }

            for group_name, model_names in groups.items():
                group_results = [res for res in results if res['name'] in model_names]
                if group_results:
                    n_models_g = len(group_results)
                    fig_g, axes_g = plt.subplots(1, n_models_g, figsize=(4 * n_models_g, 5), constrained_layout=True)
                    if n_models_g == 1: axes_g = [axes_g]
                    
                    for ax, res in zip(axes_g, group_results):
                        im = ax.imshow(res['grid'], cmap='viridis', origin='lower', extent=res['extent'], 
                                       aspect='auto', interpolation='none', vmin=vmin, vmax=vmax)
                        ax.set_xlabel('Variance')
                        ax.set_ylabel(res['ylabel'])
                    ...
                    fig_g.colorbar(im, ax=axes_g, label='Renyi-2 Entropy', location='right')
                    fig_g.suptitle(f'{group_name} Entanglement Entropy vs Variance & Hidden Size (N={N})')
                    
                    filename_g = f'Entanglement_Sweep_Variance_{group_name}_L{L}.png'
                    save_path_g = get_unique_path(save_dir, filename_g)
                    plt.savefig(save_path_g)
                    print(f"{group_name} combined plot saved to {save_path_g}")
                    plt.close(fig_g)
            ...
            for res in results:
                ...
                # Set ticks for non-linear axes
                ax_s.set_xticks(np.arange(len(res['variances'])) + 0.5)
                ax_s.set_xticklabels(res['variances'], rotation=45)
                ax_s.set_yticks(np.arange(len(res['h_vals'])) + 0.5)
                ax_s.set_yticklabels(res['h_vals'])

                fig_s.colorbar(im_s, ax=ax_s, label='Renyi-2 Entropy')
                
                fname_s = f'Entanglement_Sweep_Variance_{res["name"]}_L{L}.png'
